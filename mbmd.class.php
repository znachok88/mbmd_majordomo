...
class mbmdSocket
{
   ...
   public function ReadyDataWriteEvent()
   {
      if (!$this->dataToWrite) {
         ...
         $this->WriteLog(date('H:i:s') . ' CreateHeader(): ' . $this->dataToWrite);
   ...
   {
      $this->WriteLog(date('H:i:s') . " WriteData({$data}, {$type})");
      if (!$this->IsOffline()) {
         if ($encode) {
   ...
   {
      $this->WriteLog(date('H:i:s') . " ParseHeader(): " . $header);
